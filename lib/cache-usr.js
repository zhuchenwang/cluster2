...
var success = common.status.success,
	...
	conn = null,
	reqSock = axon.socket('req');
	pullSock = axon.socket('pull'),
	//TODO, support connection pooling to speed up the cache operations if needed.
	reconnect = function reconnect(error){
			
		reqSock.connect(3000);
		
		reqSock.server.on('connection', function (connection) {
			conn = connection;
			conn.writable = true;
		...
		reqSock.once('close', function(error){
		...
		pullSock.connect(3001);
		pullSock.on('message', getNotified);
		
		pullSock.once('close', function (error) {
			conn.writable = false;
			reconnect(error);
		});
	},
	reply = function (pack) {
		...
		if(token === PONG){//just to keep the connection open
						
			reqSock.send(common.serialize({
				'type': PONG,
				'token': PONG
			}), reply);
		}
