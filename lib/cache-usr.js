...
var success = common.status.success,
	...
	reconnect = function reconnect(error){
		...
		reqSock.on('error', function(error){

			//conn.writable = false;
			reqSock.connect(9000);
			//reconnect(error);
		});
		
		subSock.connect(9001);
		subSock.on('message', getNotified);
		
		subSock.on('error', function (error) {
			//conn.writable = false;
			subSock.connect(9001);
            //reconnect(error);
		});
