...
Worker.prototype.initialize = function(proc, options){
	var _this = this,
	...
	process.once('SIGUSR1', function(){
		process.nextTick(function(){
			emitter.to(['master']).emit(util.format('debug-%d-listening', _this.pid), _this.pid);
		});
	});
		...
		run = function(){
			...
			when(configureApp(app)).ensure(function(configured){ //configure app before listening
				...
				var server = createServer(app).listen(port, function(){
					...
					when(warmUp(app)).ensure(function(){ //warm up app after listening
...
Worker.prototype.whenStop = function(){
	...
	(function gracefully(){
		...
		if(_this.aliveConnections > 0){
            ...
            //process.reqSock.close();
            //process.pullSock.close();
